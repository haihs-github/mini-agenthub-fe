...
const GroupMembersModal = ({
  ...
  const handleRemovePendingChip = (userId) => {
    setPendingUsers((prev) => prev.filter((p) => p !== userId));
  };
  ...
  // BKAV HaiHS: Ham dieu phoi lenh thuc thi thong qua dau noi payload da duoc sua thanh dang object body duoi api file
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm select-none animate-fade-in">
        <div className="w-full max-w-md bg-[#161b26] border border-[#232d42] rounded-2xl shadow-2xl flex flex-col relative overflow-visible max-h-[90vh]">
          <div className="px-6 py-5 border-b border-[#232d42] flex justify-between items-center bg-[#111622]/30 rounded-t-2xl shrink-0">
            <div className="space-y-1">
              <h3 className="text-md font-bold text-white tracking-wide truncate pr-4">
          ...
          <div className="p-6 flex-1 flex flex-col min-h-0 space-y-5 overflow-y-auto cyber-scrollbar">
            <div className="space-y-1.5 shrink-0">
              <div className="w-full relative">
                <input
                  ...
                  className="w-full bg-[#0b0f19] border border-[#232d42] focus:border-blue-500/40 text-xs text-gray-100 rounded-xl pl-11 pr-4 py-2.5 focus:outline-none transition-all placeholder-gray-600"
                />
                <FiSearch
                  size={14}
            ...
            {hasAddPermission ? (
              <div
                className="space-y-2 shrink-0 border-t border-[#232d42]/40 pt-4 relative overflow-visible"
                ref={searchRef}
              >
                ...
                <div className="flex gap-2 w-full relative">
                  <div className="flex-1 relative">
                    <input
                      ...
                      className="w-full bg-[#0b0f19] border border-[#232d42] focus:border-blue-500/40 text-xs text-gray-100 rounded-xl pl-11 pr-10 py-2.5 focus:outline-none transition-all placeholder-gray-600"
                    />
                    <FiSearch
                      size={14}
                      className="absolute left-4 top-3.5 text-gray-500"
                  ...
                  <button
                    ...
                    className="px-4 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-800 disabled:text-gray-600 text-xs font-bold text-white rounded-xl transition-all cursor-pointer shadow-md shrink-0"
                  >
                    Add
                  </button>
                  ...
                  {isDropdownOpen && searchResults.length > 0 && (
                    <div
                      ...
                      className="cyber-scrollbar absolute left-0 right-0 mt-11 max-h-[140px] overflow-y-auto bg-[#1a202c] border border-[#232d42] rounded-xl shadow-2xl z-[110] divide-y divide-[#232d42]/60 animate-fade-in"
                    >
                      {searchResults.map((user) => (
                        <div
                          key={user.id}
                          onClick={() => handleSelectUserPending(user)}
                          className="px-4 py-2 text-xs cursor-pointer flex flex-col text-gray-300 hover:bg-gray-800 transition-colors"
                        >
                ...
                {pendingUsers.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 pt-1 animate-fade-in">
                    {pendingUsers.map((pUser) => (
                      <div
                        key={pUser.id}
                        className="flex items-center gap-1.5 bg-blue-600/10 border border-blue-500/20 text-[11px] font-semibold text-blue-400 px-2 py-1 rounded-lg"
                      >
                        ...
                        <button
                          ...
                          className="text-blue-400/40 hover:text-red-400 cursor-pointer"
                        >
                          <FiX size={10} />
                        </button>
                      </div>
            ...
            <div className="space-y-2 border-t border-[#232d42]/40 pt-4 shrink-0">
              <label className="text-[10px] font-mono font-bold tracking-widest text-gray-400 uppercase select-none">
              ...
              <div className="h-[270px] overflow-y-auto space-y-1.5 pr-1 cyber-scrollbar">
                {isLoadingMembers ? (
                  Array.from({ length: 3 }).map((_, idx) => (
                    <div
                      key={idx}
                      className="flex items-center justify-between p-2 animate-pulse bg-[#111622]/20 rounded-xl"
                    >
                  ...
                  <div className="text-center py-12 text-xs text-gray-600 italic select-none">
                  ...
                  filteredMembers.map((member) => {
                    ...
                    return (
                      <div
                        key={member.id}
                        className="flex items-center justify-between p-2 rounded-xl hover:bg-[#1e2533]/30 border border-transparent hover:border-[#232d42]/40 transition-all group"
                      >
                        <div className="flex items-center gap-3 overflow-hidden">
                          <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-[#1e293b] to-[#0f172a] border border-[#232d42] flex items-center justify-center text-[10px] font-bold text-blue-400 shrink-0">
                            {initials}
                          </div>
                        ...
                        {hasDeletePermission && (
                          <button
                            ...
                            className="p-2 text-gray-500 hover:text-red-400 rounded-lg hover:bg-red-500/5 transition-all cursor-pointer shrink-0 opacity-0 group-hover:opacity-100 focus:opacity-100"
                          >
                            <FiTrash2 size={13} />
                          </button>
                        )}
            ...
            <div className="pt-4 border-t border-[#232d42] flex justify-end items-center gap-3 bg-[#161b26] shrink-0">
              <button
                type="button"
                onClick={onClose}
                className="px-5 py-2 text-xs font-bold text-gray-400 hover:text-white border border-[#232d42] bg-[#111622] hover:bg-gray-800 rounded-xl transition-all cursor-pointer"
              >
                Cancel
              </button>
              <button
                type="button"
                onClick={onClose}
                className="px-5 py-2 text-xs font-bold text-[#0f172a] bg-[#93c5fd] hover:bg-[#7dd3fc] rounded-full transition-all shadow-md cursor-pointer"
              >
                Update Group
              </button>
            </div>
