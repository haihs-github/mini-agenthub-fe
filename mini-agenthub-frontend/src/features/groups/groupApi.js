...
// BKAV HaiHS: Bo sung API cap nhat thong tin nhom da co san trong co so du lieu - start
export const updateGroupApi = async (groupId, groupData) => {
  const response = await apiClient.put(`/groups/${groupId}`, groupData);
  return response.data;
};
// BKAV HaiHS: Bo sung API cap nhat thong tin nhom da co san trong co so du lieu - end

// BKAV HaiHS: Bo sung API quet tim kiem nhan su phan trang theo tu khoa de gan vao nhom - start
export const searchUsersApi = async (keyword, page = 1, limit = 2) => {
  const response = await apiClient.get(
    `/users/search?keyword=${keyword}&page=${page}&limit=${limit}`,
  );
  return response.data;
};
// BKAV HaiHS: Bo sung API quet tim kiem nhan su phan trang theo tu khoa de gan vao nhom - end
